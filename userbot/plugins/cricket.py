...
@icssbot.on(admin_cmd(pattern=r"score$"))
@icssbot.on(sudo_cmd(pattern=r"score$", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    chat = "@cricbuzz_bot"
...
@icssbot.on(admin_cmd(pattern=r"cric (.*)"))
@icssbot.on(sudo_cmd(pattern=r"cric (.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
