...
# =================== OWNER - ANL0KE =================== #
ALIVE_NAME = Config.ALIVE_NAME if Config.ALIVE_NAME else "@GPTHON"
...
async def logs(event):
    ...
    await tgbot.send_file(
        ...
        "logs.txt",
        reply_to=event.id,
        caption="**Heroku** 𝑮𝑷𝑻𝑯𝑶𝑵 Logs",
        buttons=[
            [Button.url("💠 𝙑𝙄𝙀𝙒 𝙊𝙉𝙇𝙄𝙉𝙀 💠", f"{url}")],
            [Button.url("💠 𝘾𝙍𝘼𝙎𝙃𝙀𝘿 💠", "t.me/GPTHON")]
        ])
...
async def settings(event):
    ...
    await tgbot.send_message(event.chat_id,
                             ...
                             buttons=[
                                     [Button.inline(
                                         "⚜️ احصل على بوت دراكون بنفسك ⚜️", data="deployme")]
                             ])
...
async def settings(event):
    await event.delete()
    await event.reply(f"⌔∮ انا دراكون مساعد {mention}. الشخصي تستطيع الاتصال بي عن طريق هذا البوت",
                             buttons=[
                                     [Button.inline(
                                         "⚜️ حصل على بوت دراكون بنفسك ⚜️", data="deployme")]
                             ])
...
async def settings(event):
    await event.edit("** ⌔∮ هنا ستجد رابط دراكون ورابط التنصيب ايضا**:",
                     buttons=[
                         [Button.url("🔗 رابط السورس🔗", url="https://github.com/hc256/hsn")],
                         [Button.url("🔗 رابط التنصيب 🔗", url="https://dashboard.heroku.com/new?template=https://github.com/taher-1/TELETHON-𝑮𝑷𝑻𝑯𝑶𝑵1")],
                         [Button.url("✨ الدعم ✨",
                                     url="https://t.me/GPTHON")]
                     ])
...
async def settings(event):
    if event.sender_id == OWNER_ID:
        ...
        await event.answer("انت لا تستطيع استخدام البوت احصل على بوتك من @GPTHON", alert=True)
