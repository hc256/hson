...
@icssbot.on(admin_cmd(pattern=r"cbutton ?(.*)", outgoing=True))
@icssbot.on(sudo_cmd(pattern=r"cbutton ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
...
@icssbot.on(admin_cmd(pattern=r"ibutton ?(.*)", outgoing=True))
@icssbot.on(sudo_cmd(pattern=r"ibutton ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
