...
async def _(event):
    ...
    if BOTLOG:
        await event.client.send_message(
            BOTLOG_CHATID, "#اعادة_التشغيل \n" "⪼ بوت دراكون في وضع اعادة التشغيل انتظر"
        )
    await edit_or_reply(
        event,
        f"⌔∮ اهلا عزيزي - {mention}\n"
        f"يتم الان اعادة تشغيل بوت دراكون يستغرق الامر 1-2 دقيقه.",
    )
...
async def _(event):
    ...
    if BOTLOG:
        await event.client.send_message(
            BOTLOG_CHATID, "#اطفاء \n" "بوت دراكون في وضع الاطفاء"
        )
    await edit_or_reply(
        event, "**جارٍ إيقاف تشغيل بوت دراكون الآن ... شغِّلني يدويًا لاحقًا**"
    )
