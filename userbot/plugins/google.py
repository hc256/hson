...
@icssbot.on(admin_cmd(outgoing=True, pattern=r"gs (.*)"))
@icssbot.on(sudo_cmd(allow_sudo=True, pattern=r"gs (.*)"))
...
@icssbot.on(admin_cmd(pattern="grs$"))
@icssbot.on(sudo_cmd(pattern="grs$", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    start = datetime.now()
...
@icssbot.on(admin_cmd(pattern=r"reverse(?: |$)(\d*)", outgoing=True))
@icssbot.on(sudo_cmd(pattern=r"reverse(?: |$)(\d*)", allow_sudo=True))
@errors_handler
async def _(img):
