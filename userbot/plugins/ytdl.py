...
@icssbot.on(admin_cmd(pattern="yt(a|v)(?: |$)(.*)", outgoing=True))
@icssbot.on(sudo_cmd(pattern="yt(a|v)(?: |$)(.*)", allow_sudo=True))
async def download_video(v_url):
    ...
    try:
        ...
        await v_url.edit("`كان هناك خطأ أثناء معالجة ما بعد.`")
        return
    ...
    if song:
        ...
        await v_url.client.send_file(
            ...
            progress_callback=lambda d, t: asyncio.get_event_loop().create_task(
                progress(d, t, v_url, itime, "Uploading..", f"{ytdl_data['title']}.mp3")
            ),
        )
        ...
        await v_url.client.send_file(
            ...
            progress_callback=lambda d, t: asyncio.get_event_loop().create_task(
                progress(d, t, v_url, itime, "Uploading..", f"{ytdl_data['title']}.mp4")
            ),
        )
...
@icssbot.on(admin_cmd(pattern="yts(?: |$)(\d*)? ?(.*)", command="yts"))
@icssbot.on(sudo_cmd(pattern="yts(?: |$)(\d*)? ?(.*)", command="yts", allow_sudo=True))
...
@icssbot.on(admin_cmd(pattern="insta (.*)"))
@icssbot.on(sudo_cmd(pattern="insta (.*)", allow_sudo=True))
async def kakashi(event):
    ...
    if "www.instagram.com" not in link:
        await edit_or_reply(
            event, "` أحتاج إلى رابط Instagram لتنزيل الفيديو الخاص به...`(*_*)"
        )
    else:
        start = datetime.now()
        icse = await edit_or_reply(event, "**جارى التحميل.....**")
    async with event.client.conversation(chat) as conv:
        try:
            ...
            await icse.edit("**خطأ: ** `ارفع الحظر من البوت`instasavegrambot` وأعد المحاولة!`")
            return
        await icse.delete()
